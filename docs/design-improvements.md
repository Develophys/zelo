...
## P3 — Polish. 19 of 20 done.

Worked in small validatable groups. ✅ marks an item that is done and covered.

- ✅ **`AppHeader.tsx:57-63` always renders the subtitle `<p>`, even when empty.** On `/assessment/result` and `/crisis` an empty 18px paragraph pushes the title above optical center on every load. **Done —** The paragraph is rendered only when there is a subtitle.
- ✅ **`ChatEmptyState.tsx:7` uses `text-justify hyphens-auto`** on a narrow phone column. Justified Portuguese in a ~52ch measure produces visible rivers. Use `text-pretty`, already used in `ChatAlerts.tsx:40`. **Done —** Now `text-pretty`.
...
- ✅ **`components/HealthBanner.tsx` is dead and dev-grade** — renders `api: {status}` and a lowercase "dismiss", the only English in a PT-BR UI, on a ~20px unlabeled button. Nothing imports it (grep confirms only its own test). Delete it before someone mounts it. **Done —** Deleted, along with `ui.store.ts`, which held only its dismissed flag. `useApiHealth` and the `check-api-health` use case/port/adapter are now unconsumed by any UI — left in place with their tests rather than removed as a side effect.
- ✅ **`ui/CardTitle.tsx:4` uses `text-lg`** — Tailwind's default 18px, off the type scale. It's the manager panel's only card heading. **Done —** A `--text-card-title: 18px` token was added rather than snapping the heading to `text-h2` (24px, too large for the panel's dense cards). Same size as before, now on the scale.
- ✅ **`chat-bubble.ts:1` hardcodes `rounded-[20px]`**, bypassing both the corner scale and the user's `data-corners` preference (`index.css:275-281`). A user who picks "Cantos: arredondados" sees every card change and the chat bubbles stay put. **Done —** A `--radius-bubble` token was added — 20px sharp, 28px under `data-corners='rounded'` — rather than reusing `card-lg`, which would have flattened the bubble to 14px and changed the chat's default look. The preference now moves the bubbles; the designed shape is unchanged. The file is `ui/message-bubble.ts` since the P2 extraction, so this fixes both transcripts at once.
- ✅ **`Skeleton.tsx:6` `animate-pulse` lacks `motion-essential`.** Under reduced motion the loading skeleton goes fully static, losing its only "something is happening" signal. Every other indefinite indicator (spinner, typing dots) has the opt-in. One class. **Done —** Added, so a skeleton keeps an opacity pulse under reduced motion like the spinner and the typing dots already did.
- ✅ **`ScaleAssessmentPage.tsx:93-95`** — "Não foi possível enviar. Selecione uma opção para tentar novamente." The user *did* select an option. Give it a retry button. **Done —** Already fixed with the review step in P0 #2 — the copy no longer tells the user to re-select an option, and there is a real retry button.
- ✅ **`lib/theme.ts:11-12` hardcodes `#f2f5f3`/`#101815`** for `theme-color` meta. They must be literal, but nothing enforces that they track `--color-canvas`. They will silently drift. **Done —** The literals stay — none of the three places can read a custom property — but `lib/theme-color.test.ts` now pins all three (theme.ts, the meta tag and the pre-paint script in index.html) to `--color-canvas`. Verified to fail on a one-digit change.
- ✅ **`PeersPage.tsx:56-61`** — "Procurando um colega disponível..." has no timeout, no elapsed indicator, and no CVV fallback. At 03:40 the realistic outcome is `no_peer_available`, which offers only "Tentar novamente". There is no crisis line on this screen at all. **Done —** **Reclassified out of Polish — this was a P1-class safety gap.** The screen a doctor reaches *to find a human* had no crisis line anywhere on it, and a 03:40 search realistically ends in `no_peer_available` offering only "Tentar novamente". The line is now quietly present in every state via a hoisted `PeerFooter`, promoted to a full action beside the retry when no colleague is found, and a search that passes 15s says so rather than sitting silent — without cancelling. `PeersPage` was added to `crisis-call-reachability.test.tsx`, so it is now held to the same sweep as the three crisis routes.
- ✅ **`PeersPage.tsx:30-35` and `:89-94`** duplicate the same "conexão sem troca de identidade" block verbatim. Hoist it. **Done —** Hoisted into `PeerFooter`, which now also carries the crisis line, so both live in one place for every branch.
- ✅ **Read notifications remain focusable dead buttons** (`ManagerNotificationsPage.tsx:64-66`). `onClick` no-ops and `cursor-pointer` is dropped, but a keyboard user still tabs every already-read row. **Done —** Only an unread row is a `<button>` now; a read one renders as a `<div>`. The existing test guarding that read/unread reaches assistive tech was kept, restated for a row that is no longer interactive — the state still has to be text, not only colour.
- ✅ **`PhoneShell.tsx:72` `<main>` carries `no-scrollbar`.** On desktop, long pages give no indication there is more below. **Done —** Now `max-md:no-scrollbar`: hidden on phones, where the OS draws an overlay scrollbar anyway, and visible from 768px up where it is the only cue a long page continues.
- ✅ **On phone, `DataTableShell` renders a bordered box containing only a toolbar**, because the `<table>` is `hidden md:table` (`DataTable.tsx:46`) while the real content is a sibling `<ul className="md:hidden">` *outside* the shell (`ManagerAdminManagersPage.tsx:318`). Search and bulk actions float detached from the list they act on. **Done —** The phone list moved inside the shell, so search and bulk actions sit with the list they act on.
- ✅ **`DataTable` supplies no mobile fallback of its own.** All three consumers provide one, but it is a convention held by call sites, not by the component. A fourth consumer that forgets renders nothing below 768px. **Done —** `mobileList` is now a **required** prop, so a fourth consumer cannot forget it — TypeScript refuses. The card markup stays with each page, since it is page-specific; only the contract and the placement moved into the component. A side effect worth knowing: the list now renders with the rows instead of sitting in the DOM as an empty `<ul>` before data arrives.
- ✅ **`ManagerInsightHistoryPage:239`** defaults the first mobile card expanded (`isDefaultOpen={index === 0}`) while the desktop table defaults all rows collapsed. Same data, two initial states. **Done —** **Reviewed — deliberately not changed.** Forcing parity was tried and reverted: four tests showed the expanded first card is doing real work. The desktop table gives date, summary and actions per row at a glance; a phone card gives date and summary, so opening the newest analysis is what makes the two carry comparable information on arrival. Collapsing it would cost a tap for the entry most people came for, to buy consistency nobody experiences — you would have to resize across 768px mid-session to notice. The reasoning now sits in the component so the next reader does not re-litigate it.
- ✅ **iPad portrait is 768px — exactly the `md` boundary.** The manager rail shows six unlabeled icons (labels are `lg:not-sr-only`, `ManagerSidebar.tsx:47`) and the collapse toggle that would help is itself `hidden lg:flex` (`SidebarHeader.tsx:62`) — unavailable at the width where it is needed. Tooltips need a 450ms long-press per icon to learn what anything is. **Done —** The rail now shows a compact label under each icon from md up, stacked at 10px and switching to the inline 14px label at lg. `collapsed` is an lg-and-up preference, so it no longer hides labels at the one width that cannot toggle them. The 450ms long-press per icon is no longer the only way to learn what anything is, which also removes the need for the lg-only collapse toggle to be reachable at md.
- ✅ **The search empty state on all three admin pages is engineer-speak** — "Nenhum resultado nos itens carregados / A busca ainda percorre apenas a lista já carregada" (`ManagerAdminManagersPage.tsx:311-314`, duplicated in Sectors and Peers). It describes the implementation, not the user's situation. **Done —** Rewritten to "Nada encontrado para esta busca / Tente outro termo ou revise a ortografia." The old hint was not merely engineer-speak but **inaccurate**: these lists load through a plain `useQuery` with no pagination and no server limit, so there is no partially-loaded list for the search to miss.
- ✅ **`getGreeting` returns "Boa noite." for 18:00–04:59.** A doctor opening at 04:30 mid-shift gets "Boa noite." The file's own comment says the goal is to meet them at the hour they're actually in — "Boa madrugada." would land better with this exact user. **Done —** 00:00–04:59 now gets "Boa madrugada." Boundary tests added and verified to fail against the previous implementation.
- ✅ **`text-eyebrow` is 12px at `0.1em` tracking**, frequently in `text-muted-2` (`SectionLabel`'s default tone). At 12px uppercase mono that sits near the AA floor and is the app's least legible recurring text. Spot-check on `canvas-alt`. **Done —** Measured: `text-muted-2` clears AA on surface (5.01:1) and canvas (4.57:1) but fails on canvas-alt (4.41:1) and surface-brand (4.16:1) in light mode; `text-muted` passes on all four. All three current call sites happened to be safe, but the **default** tone was the unsafe one. `SectionLabel` now defaults to `text-muted`, with the lighter tone renamed to an explicit `subtle` opt-in.
...
| ✅ P1 | **Fixed.** The acknowledgement now replaces the question in place: "Obrigado por dizer." plus the two routes that already exist (acolhimento, pares). "Estou bem" gets a brief confirmation rather than vanishing. Scoped to the mount — the persisted answer is still what suppresses the prompt on a later visit, so returning to Home does not re-open a conversation already had. The copy is new and normative; worth a review pass. |
| ✅ P1 | **Fixed.** `md` is `px-5`. The two `p-2!` overrides in `FollowUpCard` existed only to fight it and are gone. |
| ✅ P1 | **Fixed, and it was worse than reported.** `setState("idle")` fired synchronously right after `connect()`, so a volunteer was told *"Conectado, aguardando solicitações"* the moment the page mounted — even with a dead socket. They believed they were on duty while a doctor's request went unanswered. State is now reported by the socket's own `connect`/`connect_error`/`disconnect` events, the union gained an `error` member, and the page renders a `role="alert"` with a retry. |
| ✅ P2 | **Fixed.** The `ManagerSidebar` treatment is ported: a compact label under each icon from md up, switching to the inline label at lg, with `collapsed` (an lg-and-up preference) no longer hiding labels at the one width that cannot toggle them. |
| ✅ P1 | **Consent gate closed on the routes that collect.** `/chat`, `/assessment`, `/assessment/phq9`, `/assessment/gad7`, `/assessment/result` and `/peers` had no consent loader, so a deep link, bookmark or history entry reached the PHQ-9 or the AI chat without the disclosure the consent screen exists to make. They are gated now. **`/crisis*` is deliberately left open** — someone reaching for the CVV number must not meet a consent form first, and those screens collect nothing (`RequestHumanHandoffUseCase` is synchronous and I/O-free). A test states both halves so the exception reads as a decision rather than an oversight. |
| ✅ P1 | **The manager's AI output says how it was made.** The doctor's chat carries a permanent AI banner; the manager-facing interpretation had none — and it is the output that leaves the app as .txt or PDF and gets quoted to leadership. `MANAGER_INSIGHT_DISCLAIMER` is one shared string used by the dashboard card, the history entry and both exports, so the screen and the meeting deck cannot say different things. In the exports it travels *with* the document; on the history page it sits immediately above the download buttons, so the reader sees the provenance before choosing to take it out of the app. |
...
| ✅ P2 | **`ScoreDial` announced as fragments — fixed.** The dial now carries one `sr-only` sentence ("19 de 27. Faixa: Moderadamente grave.") with the visual split marked `aria-hidden`, so the most important output in the product no longer reads as "19", "/27", "Moderadamente grave" with the slash spoken. Its `/max` contrast (3.28:1, clearing AA only through the large-text exemption at exactly 24px) is untouched and still open. |
