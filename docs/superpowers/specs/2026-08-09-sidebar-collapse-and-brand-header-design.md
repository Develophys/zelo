...
- **Motion:** a transição de largura do `<aside>` usa `motion-safe:transition-[width] duration-200` (200ms, dentro da faixa 150–250ms recomendada para produto) — o prefixo `motion-safe:` já respeita `prefers-reduced-motion` (requisito do `PRODUCT.md`), sem transição nenhuma para quem prefere. O texto dos rótulos alterna visibilidade instantaneamente (`hidden`/visível), sem animação própria — a largura já comunica a mudança de estado, sem coreografia extra.
